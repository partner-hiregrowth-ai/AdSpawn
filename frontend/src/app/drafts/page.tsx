...
export default function DraftsPage() {
  ...
  const getAccountName = (id: string) => {
    // Backend adAccountId usually starts with 'act_'
    // Store adAccounts might have id as 'act_...' and adaccount_id as numeric
    const acc = adAccounts.find(a => a.id === id || a.adaccount_id === id || `act_${a.adaccount_id}` === id);
    return acc?.name || id;
  };
  ...
  const filteredDrafts = useMemo(() => {
    ...
    if (accountFilter !== "ALL") {
      result = result.filter(d => 
        d.adAccountId === accountFilter || 
        d.adAccountId === `act_${accountFilter}` ||
        (accountFilter.startsWith('act_') && d.adAccountId === accountFilter.substring(4))
      );
    }
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Select value={accountFilter} onValueChange={(v) => { if (v) setAccountFilter(v); }}>
                <SelectTrigger className="h-8 w-48 bg-gray-950/50 border-gray-800/60 text-xs focus:ring-0">
                  <SelectValue placeholder="All Ad Accounts" />
                </SelectTrigger>
                <SelectContent className="bg-gray-900 border-gray-800">
                  <SelectItem value="ALL" className="text-xs text-gray-300">All Ad Accounts</SelectItem>
                  {adAccounts.map(acc => (
                    <SelectItem key={acc.id} value={acc.id} className="text-xs text-gray-300">
                      {acc.name}
                    </SelectItem>
                  ))}
                </SelectContent>
              ...
              {(statusFilter !== "ALL" || accountFilter !== "ALL" || debouncedSearch) && (
                <button
                  onClick={() => { setStatusFilter("ALL"); setAccountFilter("ALL"); setSearchQuery(""); }}
                        ...
                        <div className="space-y-1.5 min-w-0">
                          ...
                          <div className="flex flex-col gap-1 text-[11px]">
                            <div className="flex gap-2 items-center text-gray-500">
                              ...
                              <span>{draft._count?.adSets} Ad Sets / {draft._count?.ads || 0} Ads</span>
                            </div>
                            <div className="text-gray-400 font-medium truncate">
                              {getAccountName(draft.adAccountId)}
                            </div>
                          </div>
                        </div>
                      </div>
            ...
            <table className="w-full min-w-[640px]">
              <thead className="sticky top-0 z-10 bg-gray-900/95 backdrop-blur-sm">
                <tr className="border-b border-gray-800/60">
                  ...
                  <th className="text-left text-[11px] font-medium text-gray-500 uppercase tracking-wider px-3 py-3">Ad Account</th>
                  <th className="text-left text-[11px] font-medium text-gray-500 uppercase tracking-wider px-3 py-3">Objective</th>
                  <th className="text-center text-[11px] font-medium text-gray-500 uppercase tracking-wider px-3 py-3">Ad Sets / Ads</th>
                  <th className="text-left text-[11px] font-medium text-gray-500 uppercase tracking-wider px-3 py-3">Status</th>
                ...
                {filteredDrafts.map((draft) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-3 py-2.5 text-xs text-gray-400 truncate max-w-[150px]">
                        {getAccountName(draft.adAccountId)}
                      </td>
                      ...
                      <td className="px-3 py-2.5 text-center text-xs text-gray-500">
                        {draft._count?.adSets} / {draft._count?.ads || 0}
                      </td>
                      <td className="px-3 py-2.5">
                        {getStatusBadge(draft.status)}
                      </td>
