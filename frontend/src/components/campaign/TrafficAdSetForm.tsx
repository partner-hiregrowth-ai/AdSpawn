...
const TRAFFIC_PERFORMANCE_GOALS = [
  { value: "LINK_CLICKS", label: "Maximize number of link clicks", recommended: true },
...
const TRAFFIC_PERFORMANCE_GOAL_LABELS: Record<string, string> = Object.fromEntries(
  TRAFFIC_PERFORMANCE_GOALS.map((g) => [g.value, g.label])
);
...
const TRAFFIC_BILLING_EVENT_LABELS: Record<string, string> = Object.fromEntries(
  TRAFFIC_BILLING_EVENTS.map((b) => [b.value, b.label])
);

// ─── Searchable dropdown (used in audience section) ───────────────────────────
...
interface TrafficAdSetFormProps {
  initialValues: Record<string, any>;
  campaignBudget?: number;
  onChange: (values: Record<string, any>) => void;
}

export function TrafficAdSetForm({
  initialValues,
  campaignBudget,
  onChange,
}: TrafficAdSetFormProps) {
  const [name, setName] = useState<string>(initialValues.name ?? "New Traffic Ad Set");
  ...
  const [performanceGoal, setPerformanceGoal] = useState(
    initialValues.optimization_goal ?? "LINK_CLICKS"
  );
  ...
  const emit = useCallback(
    (overrides: Record<string, any> = {}) => {
      ...
      const effectiveLoc = (overrides.conversionLocation ?? conversionLocation) as TrafficConversionLocationValue;
    ...
    [initialValues, name, conversionLocation, performanceGoal, billingEvent, startDate, startTime, hasEndDate, endDate, endTime]
  );
        ...
        <SectionBody>
          ...
          <FieldRow label="Conversion location">
            <Select
              ...
              onValueChange={(v) => {
                if (!v) return;
                const loc = v as TrafficConversionLocationValue;
            ...
            >
              <SelectTrigger className="w-full bg-gray-800/30 border-gray-700/40 text-sm text-gray-200 focus:border-blue-500/50">
                <span className="truncate">
                  {TRAFFIC_CONVERSION_LOCATIONS.find((l) => l.value === conversionLocation)?.label ?? conversionLocation}
                </span>
              </SelectTrigger>
          ...
          <FieldRow label="Performance goal">
            ...
            >
              <SelectTrigger className="w-full bg-gray-800/30 border-gray-700/40 text-sm text-gray-200 focus:border-blue-500/50">
                <span className="truncate">
                  {TRAFFIC_PERFORMANCE_GOAL_LABELS[performanceGoal] ?? performanceGoal}
                </span>
              </SelectTrigger>
              <SelectContent className="bg-gray-900 border-gray-800">
                {TRAFFIC_PERFORMANCE_GOALS.map((g) => (
          ...
          <FieldRow label="Billing event">
            ...
            >
              <SelectTrigger className="w-full bg-gray-800/30 border-gray-700/40 text-sm text-gray-200 focus:border-blue-500/50">
                <span className="truncate">
                  {TRAFFIC_BILLING_EVENT_LABELS[billingEvent] ?? billingEvent}
                </span>
              </SelectTrigger>
              <SelectContent className="bg-gray-900 border-gray-800">
                {TRAFFIC_BILLING_EVENTS.filter(
