...
const BUYING_TYPE_LABELS: Record<string, string> = Object.fromEntries(
  BUYING_TYPES.map((b) => [b.value, b.label])
);

// ─── Main Form ────────────────────────────────────────────────────────────────

interface EngagementCampaignFormProps {
  initialValues: Record<string, any>;
  onChange: (values: Record<string, any>) => void;
}

export function EngagementCampaignForm({ initialValues, onChange }: EngagementCampaignFormProps) {
  // ── Local state derived from initialValues ──
  const [name, setName] = useState<string>(initialValues.name ?? "New Engagement Campaign");
  const [buyingType, setBuyingType] = useState<string>(
    initialValues.buying_type ?? "AUCTION"
  );
  ...
  useEffect(() => {
    ...
    setName(initialValues.name ?? "New Engagement Campaign");
  ...
  const emit = useCallback(
    (overrides: Record<string, any> = {}) => {
      ...
      const values: Record<string, any> = {
        ...
        buying_type: overrides.buying_type ?? buyingType,
              ...
              >
                <SelectTrigger className="bg-gray-800/30 border-gray-700/40 text-sm text-gray-200 focus:border-blue-500/50">
                  <span className="truncate">
                    {BUYING_TYPE_LABELS[buyingType] ?? buyingType}
                  </span>
                </SelectTrigger>
