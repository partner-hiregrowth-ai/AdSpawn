...
interface AwarenessCampaignFormProps {
  initialValues: Record<string, any>;
  onChange: (values: Record<string, any>) => void;
}

export function AwarenessCampaignForm({ initialValues, onChange }: AwarenessCampaignFormProps) {
  // ── Local state derived from initialValues ──
  const [name, setName] = useState<string>(initialValues.name ?? "New Awareness Campaign");
  ...
  // Without this guard, every toggle/input would have its state immediately overwritten
  // by the bounced-back value (e.g. hasBudgetSchedule gets reset to false because no
  // start_time is in the payload yet). isInternalChange blocks that reset.
  ...
  useEffect(() => {
    ...
    setName(initialValues.name ?? "New Awareness Campaign");
