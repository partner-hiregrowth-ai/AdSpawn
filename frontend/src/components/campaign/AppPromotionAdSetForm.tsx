...
// Suppress unused warning — WarningBox is available for future use
void WarningBox;
...
const APP_PERFORMANCE_GOALS = [
  { value: "APP_INSTALLS", label: "Maximize app installs", recommended: true },
  { value: "APP_INSTALLS_AND_OFFSITE_CONVERSIONS", label: "Maximize app installs and conversions" },
...
interface AppPromotionAdSetFormProps {
  initialValues: Record<string, any>;
  campaignBudget?: number;
  onChange: (values: Record<string, any>) => void;
}

export function AppPromotionAdSetForm({
  initialValues,
  campaignBudget,
  onChange,
}: AppPromotionAdSetFormProps) {
  const [name, setName] = useState<string>(initialValues.name ?? "New App Promotion Ad Set");
  ...
  const [appId, setAppId] = useState<string>(
    initialValues.promoted_object?.application_id ?? ""
  );
  const [storeUrl, setStoreUrl] = useState<string>(
    initialValues.promoted_object?.object_store_url ?? ""
  );
  ...
  const emit = useCallback(
    (overrides: Record<string, any> = {}) => {
      ...
      const effectiveLoc = overrides.conversionLocation ?? conversionLocation;
      const effectiveGoal = overrides.performanceGoal ?? performanceGoal;
      const effectiveBilling = overrides.billingEvent ?? billingEvent;
      const effectiveAppId = overrides.appId !== undefined ? overrides.appId : appId;
      const effectiveStoreUrl = overrides.storeUrl !== undefined ? overrides.storeUrl : storeUrl;
      ...
      if (effectiveAppId && effectiveAppId.trim() && effectiveStoreUrl && effectiveStoreUrl.trim()) {
        values.promoted_object = {
          application_id: effectiveAppId.trim(),
          object_store_url: effectiveStoreUrl.trim(),
        };
      } else {
  ...
  const appIdHasError = appIdTouched && !appId.trim();
  const storeUrlHasError = !!(appId.trim() && storeUrlTouched && !storeUrl.trim());
  ...
  return (
    <div className="space-y-4">
      ...
      <SectionCard>
        ...
        <SectionBody>
          {/* App destination */}
          <FieldRow label="App destination">
            <Select
              value={conversionLocation}
              onValueChange={(v) => {
                if (!v) return;
                setConversionLocation(v);
                emit({ conversionLocation: v });
              }}
            >
              <SelectTrigger className="w-full bg-gray-800/30 border-gray-700/40 text-sm text-gray-200 focus:border-blue-500/50">
                <span className="truncate">
                  {APP_DESTINATION_LOCATIONS.find((l) => l.value === conversionLocation)?.label ?? conversionLocation}
                </span>
              </SelectTrigger>
              <SelectContent className="bg-gray-900 border-gray-800">
                {APP_DESTINATION_LOCATIONS.map((loc) => (
          ...
          <FieldRow label="Performance goal">
            ...
            >
              <SelectTrigger className="w-full bg-gray-800/30 border-gray-700/40 text-sm text-gray-200 focus:border-blue-500/50">
                <span className="truncate">
                  {APP_PERFORMANCE_GOALS.find((g) => g.value === performanceGoal)?.label ?? performanceGoal}
                </span>
              </SelectTrigger>
              <SelectContent className="bg-gray-900 border-gray-800">
                {APP_PERFORMANCE_GOALS.map((g) => (
          ...
          <FieldRow label="Billing event">
            ...
            >
              <SelectTrigger className="w-full bg-gray-800/30 border-gray-700/40 text-sm text-gray-200 focus:border-blue-500/50">
                <span className="truncate">
                  {APP_BILLING_EVENTS.find((b) => b.value === billingEvent)?.label ?? billingEvent}
                </span>
              </SelectTrigger>
              <SelectContent className="bg-gray-900 border-gray-800">
          ...
          {/* Application ID */}
          <FieldRow label="Application ID *" required>
            <Input
              value={appId}
              onChange={(e) => {
                setAppId(e.target.value);
                emit({ appId: e.target.value });
              }}
              onBlur={() => setAppIdTouched(true)}
              placeholder="Enter App ID (e.g. 300000000000001)"
              className={cn(
            ...
            {appIdHasError && (
              <p className="text-[11px] text-red-400 flex items-center gap-1 mt-1">
                ...
                An Application ID is required for App Promotion campaigns.
              </p>
            )}
          </FieldRow>
          ...
          <FieldRow
            label="App Store URL *"
            hint="Enter your Google Play or App Store URL"
            required
          >
            <Input
              value={storeUrl}
            ...
            {storeUrlHasError && (
              <p className="text-[11px] text-red-400 flex items-center gap-1 mt-1">
                <AlertTriangle className="w-3 h-3 shrink-0" />
                App Store URL is required when Application ID is set.
              </p>
            )}
          </FieldRow>
        </SectionBody>
