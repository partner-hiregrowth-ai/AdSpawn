...
const ENGAGEMENT_CONVERSION_LOCATION_LABELS: Record<string, string> = Object.fromEntries(
  ENGAGEMENT_CONVERSION_LOCATIONS.map((l) => [l.value, l.label])
);

const ENGAGEMENT_PERFORMANCE_GOALS = [
  { value: "POST_ENGAGEMENT", label: "Maximize post engagement", recommended: true },
  { value: "VIDEO_VIEWS", label: "Maximize video views" },
  { value: "THRUPLAY", label: "Maximize ThruPlay video views" },
...
const ENGAGEMENT_BILLING_EVENT_LABELS: Record<string, string> = Object.fromEntries(
  ENGAGEMENT_BILLING_EVENTS.map((b) => [b.value, b.label])
);
...
// Suppress unused import warning — SearchableDropdown is used via the audience section
void SearchableDropdown;

// ─── Main Component ───────────────────────────────────────────────────────────

interface EngagementAdSetFormProps {
  initialValues: Record<string, any>;
  campaignBudget?: number;
  onChange: (values: Record<string, any>) => void;
}

export function EngagementAdSetForm({
  initialValues,
  campaignBudget,
  onChange,
}: EngagementAdSetFormProps) {
  const [name, setName] = useState<string>(initialValues.name ?? "New Engagement Ad Set");

  // Section 2: Engagement
  const [conversionLocation, setConversionLocation] = useState<EngagementConversionLocationValue>(
    (initialValues.destination_type as EngagementConversionLocationValue) ?? "ON_POST"
  );
  const [performanceGoal, setPerformanceGoal] = useState(
    initialValues.optimization_goal ?? "POST_ENGAGEMENT"
  );
  ...
  const emit = useCallback(
    (overrides: Record<string, any> = {}) => {
      ...
      const effectiveLoc = (overrides.conversionLocation ?? conversionLocation) as EngagementConversionLocationValue;
      ...
      // Optional promoted_object: only include when page_id is filled
  ...
  useEffect(() => {
    ...
    setName(initialValues.name ?? "New Engagement Ad Set");
        ...
        <SectionBody>
            ...
            >
              <SelectTrigger className="w-full bg-gray-800/30 border-gray-700/40 text-sm text-gray-200 focus:border-blue-500/50">
                <span className="truncate">
                  {ENGAGEMENT_CONVERSION_LOCATION_LABELS[conversionLocation] ?? conversionLocation}
                </span>
              </SelectTrigger>
              <SelectContent className="bg-gray-900 border-gray-800">
                {ENGAGEMENT_CONVERSION_LOCATIONS.map((loc) => (
          ...
          <FieldRow label="Performance goal">
            ...
            >
              <SelectTrigger className="w-full bg-gray-800/30 border-gray-700/40 text-sm text-gray-200 focus:border-blue-500/50">
                <span className="truncate">
                  {ENGAGEMENT_PERFORMANCE_GOAL_LABELS[performanceGoal] ?? performanceGoal}
                </span>
              </SelectTrigger>
              <SelectContent className="bg-gray-900 border-gray-800">
          ...
          <FieldRow label="Billing event">
            ...
            >
              <SelectTrigger className="w-full bg-gray-800/30 border-gray-700/40 text-sm text-gray-200 focus:border-blue-500/50">
                <span className="truncate">
                  {ENGAGEMENT_BILLING_EVENT_LABELS[billingEvent] ?? billingEvent}
                </span>
              </SelectTrigger>
              <SelectContent className="bg-gray-900 border-gray-800">
          ...
          <FieldRow label="Facebook Page (optional)" hint="When provided, sets the promoted_object for this ad set.">
            <Input
              value={pageId}
              onChange={(e) => {
