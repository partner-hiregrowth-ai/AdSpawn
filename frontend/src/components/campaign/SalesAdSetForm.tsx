...
const INVENTORY_OPTIONS = [
  {
    ...
    description: "Show ads on all content that adheres to our Content Monetization Policies so you get the most reach.",
  },
  {
    value: "moderate",
  ...
  },
  {
    ...
    description: "Exclude additional sensitive content. This lowers your reach and may increase costs.",
  },
];

function InventoryFilterSubsection({ title, appliesToLabel }: { title: string; appliesToLabel: string }) {
    ...
    <div className="border border-gray-800/40 rounded-lg overflow-hidden">
      ...
      {open && (
        <div className="px-4 py-4 space-y-3">
          <p className="text-[11px] text-gray-500"><span className="text-gray-600">Applies only to: </span>{appliesToLabel}</p>
          <div className="space-y-2.5">
            {INVENTORY_OPTIONS.map((opt) => (
              <label key={opt.value} className={cn("flex items-start gap-3 p-3 rounded-lg border cursor-pointer transition-all",
                selected === opt.value ? "bg-blue-500/8 border-blue-500/30" : "bg-gray-800/15 border-gray-700/30 hover:border-gray-600/50")}>
                <input type="radio" name={`inventory-${title}`} value={opt.value} checked={selected === opt.value}
                  onChange={() => setSelected(opt.value)} className="mt-0.5 w-3.5 h-3.5 accent-blue-500 shrink-0" />
                <div className="space-y-0.5 min-w-0">
                  <div className={cn("text-xs font-semibold", selected === opt.value ? "text-blue-300" : "text-gray-300")}>
                    {opt.label}
                    {opt.value === "expanded" && <span className="ml-1.5 text-[10px] text-gray-600 font-normal">(default)</span>}
                  </div>
          ...
          <button type="button" className="text-[11px] text-blue-400 hover:text-blue-300 transition-colors flex items-center gap-1">
            See examples of excluded content <ExternalLink className="w-3 h-3" />
          </button>
        </div>
      )}
    </div>
      ...
      {open && (
        <div className="px-4 py-4">
          <InfoBox>
            ...
            <button type="button" onClick={() => toast.info("Publisher block lists are managed in Meta Business Manager.")}
              className="text-blue-400 hover:text-blue-300 transition-colors underline underline-offset-2">
              Create a publisher block list
            </button>
            <span className="text-blue-200/50"> or </span>
            <button type="button" onClick={() => toast.info("Learn more about publisher block lists in Meta Help Centre.")}
              className="text-blue-400 hover:text-blue-300 transition-colors underline underline-offset-2">
              learn more
            </button>
          </InfoBox>
    ...
    <div className="border border-gray-800/40 rounded-lg overflow-hidden">
      ...
      {open && (
        <div className="px-4 py-4 space-y-4">
          <p className="text-[11px] text-gray-500"><span className="text-gray-600">Applies only to: </span>Facebook in-stream reels and Ads on Facebook Reels</p>
          <div className="space-y-2">
            <div className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide">Exclude live videos</div>
            <label className="flex items-center gap-2.5 cursor-pointer">
              <input type="checkbox" checked={excludeLiveVideos} onChange={(e) => setExcludeLiveVideos(e.target.checked)}
                className="w-4 h-4 rounded border border-gray-600 bg-gray-800 text-blue-500 focus:ring-blue-500/30 focus:outline-none" />
              <span className="text-xs text-gray-300">Facebook in-stream reels ads</span>
            </label>
          </div>
          <div className="space-y-2">
            <div className="text-[11px] font-semibold text-gray-500 uppercase tracking-wide">Exclude nonpartner-publishers</div>
            <label className="flex items-center gap-2.5 cursor-not-allowed opacity-50">
              <input type="checkbox" disabled className="w-4 h-4 rounded border border-gray-600 bg-gray-800 text-blue-500 cursor-not-allowed" />
              <span className="text-xs text-gray-400">Facebook in-stream reels and Ads on Facebook Reels</span>
              <button type="button" title="This option requires additional account eligibility." className="text-gray-600 hover:text-gray-400 transition-colors"
                onClick={(e) => { e.preventDefault(); toast.info("This option requires account eligibility for nonpartner-publisher exclusions."); }}>
      ...
      <SectionBody>
        <div className="space-y-1.5">
          ...
          <div className="space-y-2 mt-2">
            <InventoryFilterSubsection title="In-content ads" appliesToLabel="Facebook in-stream reels and Ads on Facebook Reels" />
            <InventoryFilterSubsection title="Audience Network ads" appliesToLabel="Audience Network" />
          </div>
        </div>
...
const SALES_CONVERSION_LOCATIONS = [
...
type SalesConversionLocationValue = typeof SALES_CONVERSION_LOCATIONS[number]["value"];

const SALES_CONVERSION_LOCATION_LABELS: Record<string, string> = Object.fromEntries(
  SALES_CONVERSION_LOCATIONS.map((l) => [l.value, l.label])
);
...
const SALES_PERFORMANCE_GOALS = [
  ...
  { value: "CONVERSATIONS", label: "Maximize number of conversations" },
] as const;

const SALES_PERFORMANCE_GOAL_LABELS: Record<string, string> = Object.fromEntries(
  SALES_PERFORMANCE_GOALS.map((g) => [g.value, g.label])
);

const SALES_BILLING_EVENTS = [
  { value: "IMPRESSIONS", label: "Impressions", default: true },
  { value: "LINK_CLICKS", label: "Link clicks" },
] as const;

const SALES_BILLING_EVENT_LABELS: Record<string, string> = Object.fromEntries(
  SALES_BILLING_EVENTS.map((b) => [b.value, b.label])
);
...
interface SalesAdSetFormProps {
  initialValues: Record<string, any>;
  campaignBudget?: number;
  onChange: (values: Record<string, any>) => void;
}

export function SalesAdSetForm({
  initialValues,
  campaignBudget,
  onChange,
}: SalesAdSetFormProps) {
  const [name, setName] = useState<string>(initialValues.name ?? "New Sales Ad Set");

  // Section 2: Sales
  const [conversionLocation, setConversionLocation] = useState<SalesConversionLocationValue>(
    (initialValues.destination_type as SalesConversionLocationValue) ?? "WEBSITE"
  );
  const [performanceGoal, setPerformanceGoal] = useState(
    initialValues.optimization_goal ?? "OFFSITE_CONVERSIONS"
  );
  const [billingEvent, setBillingEvent] = useState(
    initialValues.billing_event ?? "IMPRESSIONS"
  );
  const [pixelId, setPixelId] = useState<string>(
    initialValues.promoted_object?.pixel_id ?? ""
  );
  const [customEventType, setCustomEventType] = useState<string>(
    initialValues.promoted_object?.custom_event_type ?? "PURCHASE"
  );
  ...
  const emit = useCallback(
    (overrides: Record<string, any> = {}) => {
      ...
      const effectiveLoc = (overrides.conversionLocation ?? conversionLocation) as SalesConversionLocationValue;
      const effectiveGoal = overrides.performanceGoal ?? performanceGoal;
      const effectiveBilling = overrides.billingEvent ?? billingEvent;
      const effectivePixelId = overrides.pixelId !== undefined ? overrides.pixelId : pixelId;
      const effectiveEventType = overrides.customEventType !== undefined ? overrides.customEventType : customEventType;
      ...
      // promoted_object with pixel_id is REQUIRED for OUTCOME_SALES
      if (effectivePixelId && effectivePixelId.trim()) {
        values.promoted_object = {
          pixel_id: effectivePixelId.trim(),
          custom_event_type: effectiveEventType,
        };
      } else {
  ...
  useEffect(() => {
    ...
    setName(initialValues.name ?? "New Sales Ad Set");
  ...
  const pixelIdHasError = pixelIdTouched && !pixelId.trim();
  ...
  return (
    <div className="space-y-4">
        ...
        <SectionBody>
          ...
          <FieldRow label="Conversion location">
            <Select
              ...
              onValueChange={(v) => {
                if (!v) return;
                const loc = v as SalesConversionLocationValue;
            ...
            >
              <SelectTrigger className="w-full bg-gray-800/30 border-gray-700/40 text-sm text-gray-200 focus:border-blue-500/50">
                <span className="truncate">
                  {SALES_CONVERSION_LOCATION_LABELS[conversionLocation] ?? conversionLocation}
                </span>
              </SelectTrigger>
              <SelectContent className="bg-gray-900 border-gray-800">
                {SALES_CONVERSION_LOCATIONS.map((loc) => (
          ...
          {/* Performance goal */}
          <FieldRow label="Performance goal" hint="Set your goal, such as maximizing conversions or conversion value.">
            <Select
            ...
            >
              <SelectTrigger className="w-full bg-gray-800/30 border-gray-700/40 text-sm text-gray-200 focus:border-blue-500/50">
                <span className="truncate">
                  {SALES_PERFORMANCE_GOAL_LABELS[performanceGoal] ?? performanceGoal}
                </span>
              </SelectTrigger>
              <SelectContent className="bg-gray-900 border-gray-800">
                {SALES_PERFORMANCE_GOALS.map((g) => (
          ...
          <FieldRow label="Dataset" required hint="Track actions that people take on your website.">
            ...
            {pixelIdHasError && (
              <p className="text-[11px] text-red-400 flex items-center gap-1 mt-1">
                ...
                A dataset is required for Sales campaigns.
              </p>
            )}
          </FieldRow>

          {/* Conversion event */}
          <FieldRow label="Conversion event" required hint="The action you want people to take when they see your ads.">
            <Select
              value={customEventType}
              onValueChange={(v) => {
                if (!v) return;
                setCustomEventType(v);
                emit({ customEventType: v });
              }}
            >
              <SelectTrigger className="w-full bg-gray-800/30 border-gray-700/40 text-sm text-gray-200 focus:border-blue-500/50">
                <span className="truncate">
                  {CUSTOM_EVENT_TYPES.find((e) => e.value === customEventType)?.label ?? "Select an event or search by name"}
                </span>
              </SelectTrigger>
              <SelectContent className="bg-gray-900 border-gray-800">
                {CUSTOM_EVENT_TYPES.map((e) => (
                  <SelectItem key={e.value} value={e.value} className="text-xs text-gray-300">
                    {e.label}
                  </SelectItem>
                ))}
              </SelectContent>
          ...
          {/* Set up conversion event warning */}
          <div className="flex gap-3 p-3.5 rounded-lg bg-amber-500/5 border border-l-2 border-amber-500/30 border-l-amber-500/70">
            ...
            <div className="text-xs text-amber-200/80 space-y-2 flex-1">
              <p className="font-semibold">Set up conversion event</p>
              <p>The dataset that you&apos;ve selected doesn&apos;t have any conversion events set up. Set up a conversion event to help you get better results.</p>
              <button type="button" className="text-[11px] text-amber-400 hover:text-amber-300 transition-colors underline underline-offset-2">
                Learn how to set up a conversion event
              </button>
              <div className="pt-1">
                <button type="button"
                  className="h-8 px-3 text-xs font-medium rounded-lg border border-amber-500/40 text-amber-300 hover:bg-amber-500/10 transition-colors">
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-1">
              <span className="text-xs font-medium text-gray-300">Cost per result goal</span>
            </div>
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-1">
              <span className="text-xs font-medium text-gray-300">Value rules</span>
              <Info className="w-3 h-3 text-gray-600" />
            </div>
            <p className="text-xs text-gray-500">Enabled: No</p>
          </div>
          ...
          <FieldRow label="Attribution model">
            ...
            >
              ...
              <SelectContent className="bg-gray-900 border-gray-800">
                <SelectItem value="IMPRESSIONS" className="text-xs text-gray-300">Standard</SelectItem>
                <SelectItem value="LINK_CLICKS" className="text-xs text-gray-300">Data-driven attribution</SelectItem>
              </SelectContent>
            </Select>
          </FieldRow>
          ...
          <Collapsible label="Show more settings">
            <FieldRow label="Billing event">
              ...
              >
                <SelectTrigger className="w-full bg-gray-800/30 border-gray-700/40 text-sm text-gray-200 focus:border-blue-500/50">
                  <span className="truncate">
                    {SALES_BILLING_EVENT_LABELS[billingEvent] ?? billingEvent}
                  </span>
                </SelectTrigger>
                <SelectContent className="bg-gray-900 border-gray-800">
      ...
      {/* ── 5. Audience ── */}
      <SectionCard>
        <SectionHeader title="Audience" badge={<AdvantageBadge />} />
        <SectionBody>
          ...
          <div className="border border-gray-800/40 rounded-lg overflow-hidden">
            <div className="px-4 py-3 bg-gray-800/20 flex items-center justify-between">
              <span className="text-xs font-medium text-gray-300 flex items-center gap-2">
                <Settings2 className="w-3.5 h-3.5 text-gray-500" /> Controls
              </span>
            </div>
            <div className="px-4 py-3 space-y-3">
              <div className="flex items-center gap-2 text-[11px] text-gray-500">
                <Info className="w-3.5 h-3.5 shrink-0 text-gray-600" /> No advertising settings set
              </div>
              <div className="flex items-center gap-2">
          ...
          <div className="flex items-center justify-between pt-1">
            <button type="button" className="text-[11px] text-gray-400 hover:text-gray-300 transition-colors">
              Further limit the reach of your ads
            </button>
            <button type="button" className="h-7 px-3 text-xs font-medium rounded-lg border border-gray-700 text-gray-400 hover:bg-gray-800/40 transition-colors">
              Save audience
            </button>
          </div>
        </SectionBody>
      ...
      {/* ── 7. Placements ── */}
      <SectionCard>
        <SectionHeader title="Placements" badge={<AdvantageBadge />} />
        <SectionBody>
          <p className="text-[11px] text-gray-500 leading-relaxed">
              ...
              <div className="px-4 py-3 space-y-3">
                <div className="flex items-center justify-between">
                  <div className="text-xs text-gray-400">
                    ...
                    {excludedPlacements.length === 0 ? "None" : excludedPlacements.join(", ")}
                  </div>
                </div>
                <button type="button" onClick={() => setShowPlacementModal(true)}
